...
// Timing mode (PONG_PERF=1): pump sim + offscreen draw 60x and report the
// average ms/frame. Set the "pitch" pref before running to pick the grid.
if ProcessInfo.processInfo.environment["PONG_PERF"] != nil {
    guard let v = UltracodePongView(frame: NSRect(origin: .zero, size: size),
    ...
    // Warm-up: grid rebuild, floor image, color caches.
    for _ in 0..<5 { v.animateOneFrame(); v.draw(b) }
    let t0 = CFAbsoluteTimeGetCurrent()
    for _ in 0..<60 { v.animateOneFrame(); v.draw(b) }
    let t1 = CFAbsoluteTimeGetCurrent()
    NSGraphicsContext.restoreGraphicsState()
    print(String(format: "perf: %.3f ms/frame (sim+draw, 60 frames)",
                 (t1 - t0) / 60 * 1000))
    exit(0)
}
