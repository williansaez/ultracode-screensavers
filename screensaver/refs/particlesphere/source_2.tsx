// Originkit preset `variant-2` â props baked into the default export.
...
// Speed: UI [0.1..1] â internal [0.01..0.05] (rotation speed multiplier)
function mapSpeedUiToInternal(ui: number): number {
    return mapLinear(ui, 0.1, 1.0, 0.01, 0.05)
}

// Scale: UI [0..1] â scale multiplier [0.5..3.0] (overall sphere size multiplier)
...
// Particle Size: UI [0.1..1] â size [0.01..0.1] (individual particle size)
...
// Cursor Strength: UI [0..1] â force multiplier [0..15]
...
function __OriginkitBase_ParticleSphereRefactor(__props: ParticleSphereRefactorProps) {
    ...
    // Whole-number sliders (1â10 etc.) â the engine's internal 0â1 ranges.
            ...
            if (
                ...
                for (
                    ...
                    if (mouseRef.current) {
                        ...
                        // > 0) reacts â the back layer keeps rotating untouched.
                        if (
